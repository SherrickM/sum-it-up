...
const summary = "";
  const onFormSubmit = async (event) => {
    ...
    try {
      summary = await axios
        .get(
          "http://localhost:3001/api/url/" + encodeURIComponent(formState.url))
        (setsummarizedState(summary))
        .then(console.log(summarizedState));
    } catch (err) {
      console.log(err);
    }
  };
...
export default UrlSummarySubmit;
